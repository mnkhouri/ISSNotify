...
#define RETURN_LEN_TIME 10 //time in unix timestamp format
...
#define RETURN_LEN_ISSRISETIME 10 //time in unix timestamp format
...
void loop () {
	...
	case STATE_GET_DATA:
		...
		case STATE_GET_DATA_TIME:
			//request format http://api.timezonedb.com/?format=json&lat=<LAT>&lng=<LONG>&key=<API_Key>
			timer = millis();
			Serial.println("\n>>> REQ TIME");
			...
			url_var_part[0]='\0';
			strlcat(url_var_part,"&lat=", BUFF_SIZE);
			strlcat(url_var_part,current_geoiplat_string, BUFF_SIZE);
			strlcat(url_var_part,"&lng=", BUFF_SIZE);
			strlcat(url_var_part,current_geoiplong_string, BUFF_SIZE);
			strlcat(url_var_part,"&key=", BUFF_SIZE);
			strlcat(url_var_part,apikey, BUFF_SIZE);
			print_p(PSTR("Using arguments: "));
		...
		case STATE_GET_DATA_ISS_DURATION:
			...
			timer = millis();
			Serial.println("\n>>> REQ ISS DURATION");
			fill_ip_with_dns(website_name_iss, website_ip_iss); //unexplained behaviour: ether.browseUrl makes a request to the domain that was last called by ether.dnsLookup, not the address passed to it as an argument
			url_var_part[0]='\0';
			strlcat(url_var_part,"lat=", BUFF_SIZE);
			strlcat(url_var_part,current_geoiplat_string, BUFF_SIZE);
			strlcat(url_var_part,"&lon=", BUFF_SIZE);
			strlcat(url_var_part,current_geoiplong_string, BUFF_SIZE);
			print_p(PSTR("Using arguments: "));
			Serial.println(url_var_part);
			ether.browseUrl(PSTR("/iss-pass.json?"), url_var_part, website_name_iss, my_result_cb);
			state_cb = STATE_CB_WAIT;
			state = STATE_PROCESS_DATA;
			break;
	...
	case STATE_PROCESS_DATA:
		if (millis() > timer + REQUEST_RATE){
			digitalWrite(error_led, HIGH);
			print_p(PSTR("WARNING: Timed out waiting for reply. state=="));
			Serial.print(state);
			print_p(PSTR(", state_data=="));
			Serial.println(state_data);
		...
		if(state_cb == STATE_CB_READY){
			...
			case STATE_GET_DATA_ISS_DURATION:
				...
				parseData((char*)&Ethernet::buffer[browserCallbackDatapos], keyword_issduration, current_issduration_string, RETURN_LEN_ISSDURATION);
				print_p(PSTR("set current_issduration_string="));
				Serial.println(current_issduration_string);
				state_data = STATE_GET_DATA_ISS_RISETIME;
				state = STATE_GET_DATA;
				break;
			case STATE_GET_DATA_ISS_RISETIME:
				print_p(PSTR("Got ISS risetime response\n"));
				parseData((char*)&Ethernet::buffer[browserCallbackDatapos], keyword_issrisetime, current_issrisetime_string, RETURN_LEN_ISSRISETIME);
				print_p(PSTR("set current_issrisetime_string="));
				Serial.println(current_issrisetime_string);
				state_data = STATE_GET_DATA_TIME; //we don't care about getting lat/long again, they won't change between power outs
	...
	case STATE_LIGHT_LAMP:
		print_p(PSTR("Lamp lighting state!!"));
		delay(10000);
